...
//
//   QM_ENV_FILE    path to a deployment .env to read secrets from (optional —
//                  on Fly or AWS there is no such file; set the values directly)
//   CORE_SIGNING_SECRET, ORG_ID, ADMIN_GRANTS   read from the environment when
//                  QM_ENV_FILE is unset
//   QM_CORE_URL    default http://127.0.0.1:8080; an https:// host works too
...
// A local deployment has a .env file to read; one on Fly or AWS does not — its
// secrets live in the platform's secret store, not on the machine you run this
// from. So the file is optional: set QM_ENV_FILE to read it, or supply
// CORE_SIGNING_SECRET / ORG_ID / QM_ACTOR directly in the environment.
const ENV_FILE = process.env.QM_ENV_FILE;

export const deploymentEnv = (() => {
  const map = new Map();
  if (!ENV_FILE) return map;
  let text = "";
  try {
    text = readFileSync(ENV_FILE, "utf8");
  } catch {
    throw new Error(`cannot read QM_ENV_FILE=${ENV_FILE}`);
  }
...
const fromEnv = (name) => process.env[name] ?? deploymentEnv.get(name);

export const CORE_URL = process.env.QM_CORE_URL ?? "http://127.0.0.1:8080";
export const ORG_ID = process.env.QM_ORG_ID ?? fromEnv("ORG_ID") ?? "default";
export const ACTOR = process.env.QM_ACTOR ?? (fromEnv("ADMIN_GRANTS") ?? "").split(",")[0]?.split(":")[0] ?? "";

const SECRET = fromEnv("CORE_SIGNING_SECRET");
if (!SECRET)
  throw new Error(
    "CORE_SIGNING_SECRET is not available — set it in the environment, or point QM_ENV_FILE at a deployment .env that has it",
  );

// ---------------------------------------------------------------- database

// No default container name here on purpose. A default that happens to match one
// person's machine works for them and fails everywhere else with a docker error
// that names a container the reader has never heard of.
const PG_CONTAINER = process.env.QM_PG_CONTAINER;
const PSQL_CMD = process.env.QM_PSQL;
if (!PG_CONTAINER && !PSQL_CMD)
  throw new Error(
    "no database access configured — set QM_PG_CONTAINER to the docker container running qm's postgres, or QM_PSQL to a full psql command",
  );
